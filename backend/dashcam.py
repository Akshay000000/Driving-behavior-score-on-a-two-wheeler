"""
============================================================
Varroc Eureka 3.0 — PS3  |  DASHCAM MODE
Accurate Driving Behavior Score on a Two-Wheeler
============================================================
Camera    : Bike-mounted or vehicle dashcam (MOVING camera)
Speed     : CA Kalman on ego-compensated centroid
            Fleet-shared MPP (mid-range vehicles only)
            Camera speed added for absolute ground speed
Events    : Hard brake, aggressive accel, weave, tailgate,
            sudden stop, no-helmet
            RECEDING vehicles exempt from over-speed penalty
...
VIDEO_PATH = r"./public/video3.mp4"
MODEL_PATH = "yolov8m.pt"       # yolov8m.pt for better accuracy

FRAME_W, FRAME_H = 640, 360
FPS_FALLBACK     = 30

# Lower conf — motion blur reduces detection confidence on dashcam
CONF_THRESH  = 0.28
PERSON_CONF  = 0.32
ACTIVE_CLASSES = {1, 3}         # bicycle=1, motorcycle=3
...
# Stationarity — tighter threshold (dashcam has ego-motion noise)
...
HELMET_CONFIRM_FRAMES = 30  # more frames = more stable verdict
W_NO_HELMET           = 8   # reduced weight — detection is imperfect

# Events — slightly looser thresholds for moving camera noise
...
# Tracker — shorter disappear window (vehicles exit frame fast on dashcam)
IOU_MATCH_THRESH = 0.15  # lower → catches partial overlaps after motion blur
MAX_DISAPPEARED  = 45   # longer coast → same vehicle keeps its ID
EVENT_COOLDOWN   = 18
WARMUP_FRAMES    = 15  # longer warmup suppresses Kalman cold-start false events

# Optical flow
OF_MAX_CORNERS = 200; OF_QUALITY = 0.01; OF_MIN_DIST = 6
OF_WIN_SIZE    = (21, 21)
    ...
    def update(self,gray,tracks,person_boxes):
        for tid,td in tracks.items():
            ...
            if len(h)>=HELMET_CONFIRM_FRAMES//2:
                # Require 70% of frames to show no-helmet before flagging
                self.status[tid]="HELMET" if sum(h)/len(h)>=0.45 else "NO_HELMET"
...
class KalmanVelocityEstimator:
    PROCESS_NOISE=0.8; MEASURE_NOISE=28.0
    def __init__(self):
...
class EgoMotionEstimator:
    ...
    def update(self,gray):
        ...
        if self._prev_gray is None or self._prev_pts is None or len(self._prev_pts)<10:
            self._prev_pts=cv2.goodFeaturesToTrack(gray,maxCorners=OF_MAX_CORNERS,
                qualityLevel=OF_QUALITY,minDistance=OF_MIN_DIST,mask=self._bg_mask(h,w))
            self._prev_gray=gray.copy();return 0.0,0.0
        npts,st,_=cv2.calcOpticalFlowPyrLK(self._prev_gray,gray,self._prev_pts,None,
            winSize=OF_WIN_SIZE,maxLevel=3)
        good=st.ravel()==1
        if good.sum()<5:
            self._prev_pts=None;self._prev_gray=gray.copy();return self.ego_dx,self.ego_dy
...
class FleetContext:
    ...
    def update(self,behaviors):
        ...
        # Mid-range MPP only (5-30m away) — close/far vehicles skew the scale
    ...
    def update(self,det_boxes):
        ...
        for di in unm_d:
            ...
            if bti is not None and bd<mxd:
                ...
                # Re-ID: before creating a new track, check if any coasting
                # track (disappeared > 0) is within re-ID radius.
                # This prevents 1 vehicle getting 3 IDs after brief misses.
                REID_RADIUS = math.hypot(FRAME_W,FRAME_H) / 5
                reused = False
                for old_tid, old_td in self.tracks.items():
                    if self.disappeared[old_tid] == 0: continue
                    d = math.hypot(dcx-old_td['cx'], dcy-old_td['cy'])
                    if d < REID_RADIUS:
                        box=det_boxes[di];cx,cy=self._cen(box)
                        self.tracks[old_tid]={'box':box,'cx':cx,'cy':cy}
                        self.disappeared[old_tid]=0;reused=True;break
                if not reused: self._register(det_boxes[di])
        for ti in unm_t:
...
class ByteTrackWrapper:
    def __init__(self,fps):
        self._bt=ByteTrack(track_thresh=0.25,track_buffer=25,match_thresh=0.8,frame_rate=int(fps))
...
class RiderBehavior:
    ...
    def _check_stationary(self,comp_dx,comp_dy):
        ...
        return math.hypot(comp_dx,comp_dy)<STATIONARY_DISP_PX and area_stable
    ...
    def update(self,cx,cy,box,comp_dx,comp_dy,fleet,camera_speed_mps=0.0,cls=3):
        ...
        # Kalman on ego-compensated centroid
        ...
        raw_spd_abs=cam_c if self.is_stationary else min(raw_spd_rel+cam_c,prox_cap)

        raw_fa=(raw_spd_rel-self._prev_speed)/self.dt
        ...
        # Receding vehicles exempt — they read high relative speed by nature
...
def draw_footer(frame,n,fps):
    ov=frame.copy();cv2.rectangle(ov,(0,FRAME_H-18),(FRAME_W,FRAME_H),(10,10,10),-1)
    cv2.addWeighted(ov,0.6,frame,0.4,0,frame)
    cv2.putText(frame,f"Varroc Eureka 3.0 - PS3  |  DASHCAM  |  v5",
...
def print_report(all_time,n,fps):
    ...
    COLS=[("ID",5,"<"),("Score",7,">"),("km/h",7,">"),("Brakes",7,">"),
          ...
          ("Stunt",6,"^"),("Rating",8,"^")]
    def div(l="+-",m="-+-",r="-+",f="-"):
        return "+" + "+".join(f*(w+2) for _,w,_ in COLS) + "+"
    def row(vals):
        return "|"+"│".join(f" {v:{a}{w}} " for (_,w,a),v in zip(COLS,vals))+"|"
    print();print("+"+"─"*(W-2)+"+")
    print("|"+"VARROC EUREKA 3.0  --  PS3  |  DASHCAM REPORT  v5".center(W-2)+"|")
...
def main():
    ...
    ego=EgoMotionEstimator();fleet=FleetContext();helmet_det=HelmetDetector()
    ...
    while True:
        ...
        if behaviors:
            ...
            cam_spd=min(math.hypot(edx,edy)*cam_mpp*fps,MAX_CAMERA_SPEED_MPS)
        else: cam_spd=0.0
        ...
        for tid,td in active.items():
            ...
            b.update(cx,cy,td['box'],comp_dx,comp_dy,fleet,cam_spd,get_cls(td['box']))
            all_time[tid]=b
